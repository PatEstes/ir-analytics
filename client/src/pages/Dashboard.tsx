 ...
 * Main analytics dashboard with 6 tabs + cross-filter bar:
...
function EmptyFilterState({ message }: { message?: string }) {
  return (
    <div className="panel p-12 text-center">
      <FilterX className="w-10 h-10 text-muted-foreground mx-auto mb-3 opacity-50" />
      <p className="text-sm text-muted-foreground">{message || "No data matches the current filters. Try adjusting your selections."}</p>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-background grid-dots">
      ...
      <main className="container py-6">
        <Tabs value={activeTab} onValueChange={(v) => navigate(`/dashboard/${v}`)}>
          <TabsList className="bg-secondary/50 border border-border mb-4 flex-wrap h-auto gap-1 p-1">
            {TABS.map((t) => (
          ...
          <FilterBar
            ...
            availableProgramLevels={availableProgramLevels}
          ...
          {isFiltered && (
            <div className="mb-4 px-3 py-2 rounded-lg bg-primary/5 border border-primary/20 text-xs text-primary flex items-center gap-2">
              ...
              <span>
                Showing filtered results: <strong>{themeSummary.length}</strong> of {result.themeSummary.length} themes
                {filters.institutions.length > 0 && ` · ${filters.institutions.length} institution${filters.institutions.length > 1 ? "s" : ""}`}
                {filters.programLevels.length > 0 && ` · ${filters.programLevels.length} level${filters.programLevels.length > 1 ? "s" : ""}`}
                {filters.schools.length > 0 && ` · ${filters.schools.length} school${filters.schools.length > 1 ? "s" : ""}`}
              </span>
            </div>
          )}
          ...
          <TabsContent value="summary">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              ...
              <StatCard label="Themes Shown" value={themeSummary.length} icon={BarChart3} accent />
              ...
              <>
                <div className="grid lg:grid-cols-3 gap-6">
                  <div className="lg:col-span-2 panel p-6">
                    ...
                    <ResponsiveContainer width="100%" height={Math.max(250, themeSummary.length * 50)}>
                  ...
                  <div className="panel p-6">
                    ...
                    <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
                      {byInstitution.length === 0 ? (
                        ...
                        byInstitution.slice(0, 30).map((r, i) => (
                ...
                <div className="panel p-6 mt-6">
                  ...
                  <ResponsiveContainer width="100%" height={350}>
                    <BarChart data={(() => {
                      const groups = filters.programLevels.length > 0 ? filters.programLevels : ["Undergraduate", "Graduate", "Doctoral", "Certificate"];
                      ...
                      <Tooltip content={<CustomTooltip />} />
                      <Legend />
                      {(filters.programLevels.length > 0 ? filters.programLevels : ["Undergraduate", "Graduate", "Doctoral", "Certificate"]).map((g, i) => (
                        <Bar key={g} dataKey={g} fill={[CYAN, EMERALD, AMBER, ROSE][i % 4]} radius={[2, 2, 0, 0]} />
                      ))}
                    </BarChart>
                  </ResponsiveContainer>
                </div>
                {/* By School */}
                {(filtered.bySchool.length > 0) && (
                  <div className="panel p-6 mt-6">
                    <h2 className="text-lg font-semibold mb-4" style={{ fontFamily: "var(--font-heading)" }}>Themes by School</h2>
                    <ResponsiveContainer width="100%" height={350}>
                      <BarChart data={(() => {
                        const groups = filters.schools.length > 0 ? filters.schools : ["School of Business", "School of Education", "School of Engineering", "School of Arts & Sciences"];
                        return themeSummary.map((t) => {
                          const row: any = { name: t.name };
                          groups.forEach((g) => {
                            const match = filtered.bySchool.find((r) => r.topicName === t.name && r.group === g);
                            row[g] = match?.count || 0;
                          });
                          return row;
                        });
                        ...
                        <Tooltip content={<CustomTooltip />} />
                        <Legend />
                        {(filters.schools.length > 0 ? filters.schools : ["School of Business", "School of Education", "School of Engineering", "School of Arts & Sciences"]).map((g, i) => (
                          <Bar key={g} dataKey={g} fill={[CYAN, EMERALD, AMBER, ROSE][i % 4]} radius={[2, 2, 0, 0]} />
                        ))}
                      </BarChart>
                    </ResponsiveContainer>
              ...
              <>
                <div className="grid lg:grid-cols-2 gap-6 mb-6">
                  <div className="panel p-6">
                    ...
                    <ResponsiveContainer width="100%" height={Math.max(250, themeSentiment.length * 50)}>
                      <BarChart data={[...themeSentiment].sort((a, b) => a.avgCompound - b.avgCompound)} layout="vertical" margin={{ left: 20 }}>
                        ...
                        <Bar dataKey="avgCompound" name="Avg Sentiment">
                          {[...themeSentiment].sort((a, b) => a.avgCompound - b.avgCompound).map((entry, i) => (
              ...
              <>
                ...
                <div className="panel p-6">
                  ...
                  {emergingThemes.filter((e) => e.emerging).length === 0 ? (
                    <p className="text-muted-foreground text-sm">No emerging themes detected{isFiltered ? " in the filtered dataset" : " in the current dataset"}.</p>
                  ) : (
          ...
          <TabsContent value="validation">
            {validation.filter((v) => v.topic >= 0).length === 0 ? (
              <EmptyFilterState />
            ) : (
              <>
                <div className="grid lg:grid-cols-2 gap-6 mb-6">
                  <div className="panel p-6">
                    ...
                    <ResponsiveContainer width="100%" height={Math.max(250, validation.filter((v) => v.topic >= 0).length * 45)}>
                      <BarChart data={validation.filter((v) => v.topic >= 0).sort((a, b) => (a.coherence || 0) - (b.coherence || 0))} layout="vertical" margin={{ left: 20 }}>
                        ...
                        <Bar dataKey="coherence" name="Coherence">
                          {validation.filter((v) => v.topic >= 0).sort((a, b) => (a.coherence || 0) - (b.coherence || 0)).map((entry, i) => (
